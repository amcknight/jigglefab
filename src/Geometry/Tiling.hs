...
toWedges os (Edge s@(Seg p1 p2) is) = catMaybes [buildWedge p1 p2 cps o1, buildWedge p1 p2 cps o2]
  where
...
buildWedge :: Chem c => Position -> Position -> CrossPoints -> Orb c -> Maybe Wedge
buildWedge p1 p2 (TwoCross c1 c2) (Orb o ch) = Just $ case (compare (distSq p1 o) 1, compare (distSq p2 o) 1) of
  (GT, GT) -> let (s1, s2, s3) = sort4 p1 (p2,c1,c2) in Tri o s1 s2 c -- Both outside but cross
  (GT, _) -> let (s1, s2, s3) = sort4 p1 (p2,c1,c2) in Tri o s1 s2 c -- Left out right in
  (_, GT) -> let (s1, s2, s3) = sort4 p2 (p1,c1,c2) in Tri o s1 s2 c -- Right out left in
  (_, _) -> Tri o p1 p2 c -- Both in
  where c = chemColor ch
buildWedge _ _ AllCross _ = error "Impossible for a circle and line to share all points"
buildWedge _ _ _ _ = Nothing
