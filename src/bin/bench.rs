// bench is a native-only binary. The bench module (chains, runner, output)
// is itself gated to non-wasm in lib.rs because chains.rs uses load_chemistry.
// For the wasm target this bin compiles to an empty main so trunk's
// `cargo build --target wasm32` doesn't choke on it.
...
mod native {
    ...
    use jigglefab::bench::{
        format_csv, format_markdown, run_scenario, BenchArgs, DisconnectedChains, Scenario,
        ScenarioResult,
    };
