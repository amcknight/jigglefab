...
fourChains :: Chemy c => Radius -> c -> R (Model c)
...
chainModel :: Chemy c => Radius -> Position -> Position -> c -> R (Model c)
chainModel rad from to ch = do
  c <- chainForm rad 150 1 from to ch
  pure $ buildModel rad $ box (V (-300) (-1000)) (V 1000 500) <> c

randomLinearModel :: Chemy c => Radius -> Position -> Position -> Int -> c -> R (Model c)
...
wireModel :: Chemy c => Radius -> Float -> Vector -> Vector -> c -> R (Model c)
